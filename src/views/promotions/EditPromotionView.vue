<template>
  <div class="edit-promotion-page">

    <div class="page-header mb-6">
      <h1 class="text-2xl font-bold">Edit Promotion</h1>
      <p class="text-gray-600 mt-2">Modify details of an existing promotion</p>
    </div>
...
const {data: promotionData, isPending: isPromotionLoading, isError: isPromotionError} = useQuery({
  queryKey: [`promotions/${route.params.promotionId}`],
  queryFn: async () => {
    const {data} = await axios.get(`/promotions/${promotionId}`);
    return data;
  },
  enabled: computed(() => !!promotionId)
})
watch(() => promotionData.value, (data) => {
  if (data && data.result) {
    promotion.value.name = data.result.name;
    promotion.value.discountPercentage = data.result.discountPercentage;
    promotion.value.startDate = new Date(data.result.startDate);
    promotion.value.endDate = new Date(data.result.endDate);
    promotionTranslations.value = data?.result?.promotionTranslations?.length > 0 ? data.result.promotionTranslations : [];
    const set = new Set(promotionTranslations.value.map(translation => translation.language));
    ...
    data.result.promotionDetails.forEach(detail => {
      selectedProducts.value.push({
        id: detail?.productId,
        name: detail?.productName,
        categoryName: detail?.productCategoryName,
...
const updateMutation = useMutation({
  mutationFn: async (promotionData) => {
    ...
    return await axios.put('/promotions', promotionData);
  },
  onSuccess: () => {
...
const updatePromotion = async () => {
  ...
  confirm({
    ...
    message: 'Are you sure you want to update this promotion? This will affect all products associated with it.',
    okText: 'Update',
    cancelText: 'Cancel',
    child: {
      okButton: {
...
const proceedWithUpdate = async () => {
  try {
    ...
    await updateMutation.mutateAsync(promotionData);
  } catch (error) {
