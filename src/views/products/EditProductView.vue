...
const hasEmptyTranslationSpecs = (translation) => {
  ...
  if (!translation.translatedSpecs) return false;
  if (!translation.translatedSpecs.items || !Array.isArray(translation.translatedSpecs.items)) return false;

  if (translation.translatedSpecs.items.length === 0) return false;

  return translation.translatedSpecs.items.some(spec => {
    if (!spec) return false;
    return (spec.key && spec.key.trim() !== '' && (!spec.value || spec.value.trim() === '')) ||
...
const hasDuplicateTranslationSpecKeys = (translation) => {
  ...
  if (translation.translatedSpecs.items.length < 2) return false;
...
const isTranslationSpecKeyDuplicate = (translation, currentKey, currentIndex) => {
  ...
  if (!translation.translatedSpecs) return false;
  if (!translation.translatedSpecs.items || !Array.isArray(translation.translatedSpecs.items)) return false;
  if (!currentKey || !currentKey.trim()) return false;

  return translation.translatedSpecs.items.some((spec, index) =>
      spec && spec.key && spec.key.trim() === currentKey.trim() && index !== currentIndex
  );
};
...
const addTranslationSpec = (translation) => {
  if (!translation || !translation.translatedSpecs) {
    console.error('Invalid translation object');
    return;
  }

  if (!translation.translatedSpecs.items) {
    translation.translatedSpecs.items = [];
  }

  const hasIncompleteSpecs = translation.translatedSpecs.items.some(spec => {
    if (!spec) return false;
    return (!spec.key || spec.key.trim() === '') ||
...
const addNewTranslationSpecKey = (newKey, translation, index) => {
  ...
  if (!translation || !translation.translatedSpecs || !translation.translatedSpecs.items) {
    console.error('Invalid translation object');
    return;
  }

  if (index < 0 || index >= translation.translatedSpecs.items.length) {
    console.error('Invalid index for translation spec');
    return;
  }
...
const {isPending: isCategoriesLoading} = useQuery({
  queryKey: ['categories'],
  queryFn: async () => {
    const {data} = await axios.get('/categories');
    return categoryOptions.value = data?.result?.filter(category => category.status) || [];
  }
});
...
const fetchLanguageSpecs = async (language) => {
  try {
    const {data} = await axios.get(`/products/specs/${language}`);
    const result = {
      keys: new Set(),
      keyValueMap: {}
    };
...
const getLanguageSpecKeys = async (language) => {
  const specs = await getLanguageSpecs(language);
  return specs.keys || [];
};

const getLanguageValuesForKey = async (language, key) => {
  if (!key) return [];

  const specs = await getLanguageSpecs(language);
  return specs.keyValueMap[key] || [];
};
...
const productMutation = useMutation({
  mutationFn: async (productData) => {
    ...
    const {data} = await axios.put(`/products`, finalPayload);
    return data;
  },
  onSuccess: () => {
    notify({
  ...
  const productData = {
    ...
    productTranslations: productTranslations.value.filter(translation =>
            translation && (
                ...
                (translation.translatedSpecs &&
                    translation.translatedSpecs.items &&
                    Array.isArray(translation.translatedSpecs.items) &&
                    translation.translatedSpecs.items.length > 0)
            )
    ).map(translation => {

      const specsObject = {};

      if (translation.translatedSpecs &&
          translation.translatedSpecs.items &&
          Array.isArray(translation.translatedSpecs.items)) {

        translation.translatedSpecs.items.forEach(spec => {
          if (spec && spec.key && spec.value && spec.key.trim() && spec.value.trim()) {
            specsObject[spec.key.trim()] = spec.value.trim();
          }
        });
      }
...
const {data: productData, isPending: isProductLoading, isError: isProductError} = useQuery({
...
const initData = (dataProduct) => {
  product.value.sku = dataProduct.sku || '';
  product.value.stock = dataProduct?.stock || 0;
  product.value.sold = dataProduct?.sold || 0;
  product.value.status = dataProduct.status;
  product.value.name = dataProduct.name || '';
  product.value.categoryId = dataProduct.categoryId;
  product.value.categoryStatus = dataProduct.categoryStatus;
  if (!dataProduct.categoryStatus) {
    categoryOptions.value.unshift({
      id: dataProduct.categoryId,
      name: `${dataProduct.categoryName} (Inactive)`,
    });
  }
  product.value.brand = dataProduct.brand || '';
  product.value.price = dataProduct.price || '';
  product.value.description = dataProduct.description || '';

  try {
    productSpecs.value = Object.entries(JSON.parse(dataProduct.specs || '{}')).map(([key, value]) => ({
      key,
      value
    }));

    if (productSpecs.value.length === 0) {
      productSpecs.value = [{key: '', value: ''}];
    }
  } catch (e) {
  ...
  thumbnail.value = {
    url: dataProduct.thumbnail || '',
    file: null
  };

  productImages.value = (dataProduct.productImages || []).map(image => ({
    url: image.url || '',
    file: null
  }));

  if (dataProduct.productTranslations?.length > 0) {
    productTranslations.value = dataProduct.productTranslations.map(translation => {
      let specs = {};
      try {
        specs = JSON.parse(translation.translatedSpecs || '{}');
      } catch (e) {
        console.error(`Error parsing translation specs for ${translation.language}:`, e);
        specs = {};
      }
  ...
  const set = new Set(productTranslations.value.map(translation => translation.language));
  localeStore.availableLocales.forEach(locale => {
    if (!set.has(locale)) {
      productTranslations.value.push({
        language: locale,
...
<template>
  <va-card class="mb-6">
    <va-card-title>Edit Product</va-card-title>
    <va-card-content>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div class="col-span-1 md:col-span-2 mb-2">
            ...
            <div class="text-gray-700">
              <span v-if="product.status">Product is active and visible to customers</span>
              <span v-else>Product is inactive and hidden from customers</span>
            </div>
          </div>
          ...
          <va-select
              ...
              :error="!product.categoryStatus && product.categoryId === categoryOptions[0]?.id"
              :dirty="!product.categoryStatus && product.categoryId === categoryOptions[0]?.id"
              placeholder="Select category"
              no-options-text="No active categories found, go to categories to add one"
              required
          />
                ...
                <div class="border border-gray-200 rounded-lg p-4 bg-white">
                  ...
                  <div v-if="translation.translatedSpecs.items.length > 0"
                    ...
                    >
                      <div class="flex-1 pr-2">
                        <va-select
                            v-if="languageSpecs[translation.language]?.keys"
                            v-model="spec.key"
